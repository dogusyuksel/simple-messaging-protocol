...
int main(int argc, char **argv)
{
	...
	sipc_register("App_B_To_Itself", &my_callback);
	...
	for (i = 0; i < 2; i++) {
		...
		sipc_send_data("App_A_Registered_title", send_buf, strlen(send_buf));

		memset(send_buf, 0, sizeof(send_buf));
		snprintf(send_buf, sizeof(send_buf), (char *)"data %d to myself", (10 - i));
		sipc_send_data("App_B_To_Itself", send_buf, strlen(send_buf));
	}
